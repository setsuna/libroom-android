...
class MainViewModel @Inject constructor(
    ...
    fun checkDeviceAuthorization() {
        viewModelScope.launch {
            val deviceCode = EncryptionUtils.getUniqueDeviceCode()
            println("checkDeviceAuthorization response:${deviceCode}")
            val result = updateDeviceToken(UpdateDeviceToken.Params(EncryptionUtils.getDeviceType().toString(),deviceCode))
